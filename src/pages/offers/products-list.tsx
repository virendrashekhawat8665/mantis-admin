...
function ReactTable({ columns, data, getHeaderProps, renderRowSubComponent }: Props) {
  ...
  const handleAddProduct = () => {
    history(`/offers/add-new-product`);
  };
    ...
    () => [
      ...
      // },
      {
        ...
        Cell: ({ value }: any) => {
          switch (value) {
            case 'Complicated':
              return <Chip color="warning" label="Flight" size="small" variant="light" />;
            case 'Relationship':
              return <Chip color="success" label="Hotels" size="small" variant="light" />;
            case 'Single':
            default:
              return <Chip color="info" label="Trips" size="small" variant="light" />;
          }
        }
      },
      {
